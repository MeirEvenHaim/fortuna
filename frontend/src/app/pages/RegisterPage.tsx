  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok && data.token) {
        ...
        setError(data.error || 'שגיאה בהרשמה. ייתכן והאימייל כבר קיים במערכת.');
      }
    } catch (err) {
    ...
    <div className="min-h-screen bg-[#FDFCFB] flex items-center justify-center relative overflow-hidden p-4" dir="rtl">
      ...
      <div className="w-full max-w-md relative z-10 pt-10 pb-10">
        <div className="text-center mb-10">
          ...
          <h1 className="text-3xl font-black text-gray-900 mb-2">יצירת חשבון חדש ✨</h1>
          <p className="text-gray-500 font-medium">הצעד הראשון לחיבור הזוגי שלכם.</p>
        </div>
        ...
        <div className="bg-white/70 backdrop-blur-xl rounded-3xl p-8 border border-white shadow-2xl">
          ...
          <div className="mt-8 pt-8 border-t border-gray-100 text-center">
            <p className="text-gray-500 mb-4">כבר יש לכם חשבון?</p>
            <Link to="/login" className="text-pink-600 font-bold hover:text-pink-700 flex items-center justify-center gap-1 group">
              התחברו כאן
